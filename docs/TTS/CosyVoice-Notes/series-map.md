...
| [DiTAR - 连续语音表示的扩散 Transformer 自回归建模](ditar.md) | 离散 speech token 存在量化和 token 稳定性瓶颈 | 连续 VAE latent；patch-based AR；LocDiT；LM guidance | 弱化离散码本瓶颈，论文报告 WER/质量/计算量折中强 | 未开源；没有和 CosyVoice 3 直接比较；流式工程细节不足 |
...
![CosyVoice tokenizer 演进图](assets/cosyvoice-tokenizer-evolution-gpt-v3.png)
...
6. 再读 [DiTAR - 连续语音表示的扩散 Transformer 自回归建模](ditar.md)：作为 CosyVoice 离散 token 路线的对照，重点看 continuous latent patch、LocDiT 和 LM guidance。
7. 回到 Anki 卡片复习 FSQ、Conditional Flow Matching、Gumbel-Softmax、Text Normalization、Speech Tokenizer。
